...
input ENUM_TIMEFRAMES BreakoutTimeframe = PERIOD_M5; // Timeframe para detectar rompimento da entrada principal
input int      OpeningChannelBars = 4;      // Quantidade de velas para compor o canal de abertura
...
input bool     EnableEntryVerboseLogs = true; // Logs de diagnostico para motivos de nao abertura
input int      EntryVerboseIntervalSeconds = 60; // Intervalo minimo entre logs repetidos de diagnostico de entrada
...
input double   InitialDepositReference = 0.0; // Deposito inicial de referencia (0 = usar saldo inicial capturado)
...
input EDrawdownPercentReference DrawdownPercentReference = DD_REF_DAY_BALANCE; // Base de referencia para DD percentual
input bool     ForceDayBalanceDDWhenUnderInitialDeposit = true; // Forcar base DD no saldo do dia se saldo inicio dia < deposito inicial
...
input bool     EnableVerboseDDLogs = true;    // Logs verbosos de DD/DD+LIMIT
input int      DDVerboseLogIntervalSeconds = 60; // Intervalo minimo entre logs verbosos de DD
...
input group "--- PCM - Ativacao e Fluxo ---"
input bool     EnablePCM = false;            // Habilitar estrategia PCM
...
input group "--- PCM - Canal ---"
input bool     PCMUseMainChannelRangeParams = true; // Usar parametros de range do bloco Gatilho e Canal
input double   PCMMinChannelRange = 2.5;     // Range minimo do canal para PCM (quando nao usar parametros principais)
input double   PCMMaxChannelRange = 14.99;   // Range maximo do canal para PCM (quando nao usar parametros principais)
input double   PCMSlicedThreshold = 15.0;    // Threshold de SLD para PCM (quando nao usar parametros principais)
...
input double   PCMNegativeAddTPDistancePercent = 100.0; // Distancia TP apos ADON em PCM (% da dist. da entrada media ate o SL)

input group "--- PCM - Gestao de Posicao ---"
input bool     BreakEven = false;            // Break even
input double   PCMBreakEvenTriggerPercent = 50.0; // Percentual da distancia ate TP para ativar break even
input bool     TraillingStop = false;        // Trailling stop

input group "--- PCM - Diagnostico ---"
input bool     EnablePCMVerboseLogs = true;  // Logs verbosos de diagnostico PCM (pode reduzir performance)
input int      PCMVerboseIntervalSeconds = 60; // Intervalo minimo entre logs repetitivos do PCM (segundos)
...
input bool     KeepPositionsOverWeekend = true; // Manter posicoes abertas no fim de semana
...
datetime g_channelDefinitionTime = 0;  // Fechamento da ultima vela usada para definir o canal
...
datetime g_entryVerboseLastNoBreakoutLogTime = 0;
datetime g_entryVerboseLastFirstEntryHourBlockLogTime = 0;
datetime g_entryVerboseLastChannelInvalidLogTime = 0;
...
void LogNoTrade(string reason,
               ...
               double stopLoss,
               double takeProfit,
               double missingToLimitPoints,
               double rrMaxReached,
               double rrMinRequired,
               bool pcmArmedFromNoTrade);
...
void ConfigureTradeFillingMode(string context)
{
   ...
   Print("INFO: filling mode configurado para ", EnumToString(g_tradeFillingMode),
         " [", context, "]");
}
...
bool IsEntryVerboseEnabled()
{
   return EnableEntryVerboseLogs;
}

int ResolveEntryVerboseIntervalSeconds()
{
   if(EntryVerboseIntervalSeconds < 1)
      return 1;
   return EntryVerboseIntervalSeconds;
}

bool ShouldEmitEntryVerbose(datetime &lastEmitTime)
{
   if(!IsEntryVerboseEnabled())
      return false;

   datetime now = TimeCurrent();
   int intervalSeconds = ResolveEntryVerboseIntervalSeconds();
...
void PrintEntryVerboseRateLimited(string message, datetime &lastEmitTime)
{
   if(!ShouldEmitEntryVerbose(lastEmitTime))
      return;
   Print("ENTRY DEBUG: ", message);
}
...
void EnsurePCMVisualizationMirrorChart()
{
   ...
   if(g_pcmActiveTimeframe != PERIOD_M1)
   {
      if(g_pcmMainChartForcedToM1 && g_visualBaseTimeframe != PERIOD_M1)
      {
         if(ChartSetSymbolPeriod(0, _Symbol, g_visualBaseTimeframe))
         {
            ...
            Print("INFO: visualizacao principal restaurada para ", EnumToString(g_visualBaseTimeframe), ".");
         }
      }
      return;
   }
   ...
   if(!needPCMView)
   {
      if(g_pcmMainChartForcedToM1 && g_visualBaseTimeframe != PERIOD_M1)
      {
         ...
         {
            g_pcmMainChartForcedToM1 = false;
            Print("INFO: visualizacao principal restaurada para ", EnumToString(g_visualBaseTimeframe), " apos ciclo PCM.");
         }
      }
      return;
   }
   ...
   if(currentMainTf != PERIOD_M1 && !g_pcmMainChartSwitchUnavailable)
   {
      if(ChartSetSymbolPeriod(0, _Symbol, PERIOD_M1))
      {
         ...
         Print("INFO: visualizacao principal alternada para M1 durante ciclo PCM.");
      }
      else
      {
         g_pcmMainChartSwitchUnavailable = true;
         Print("INFO: ambiente tester visual nao permite alternar timeframe do grafico principal via EA.");
      }
   }
   ...
   if(g_pcmMirrorChartId > 0)
   {
      ...
      Print("INFO: chart espelho M1 aberto para visualizacao PCM. chart_id=", g_pcmMirrorChartId);
   }
   else
   {
      g_pcmMirrorChartOpenUnavailable = true;
      Print("INFO: ambiente tester visual nao permite abrir aba M1 adicional via EA.");
   }
}
...
int ResolveOpeningChannelBars()
{
   if(OpeningChannelBars <= 0)
      return 4;
   return OpeningChannelBars;
}
...
ENUM_TIMEFRAMES ResolveMainBreakoutTimeframe()
{
   if(BreakoutTimeframe == PERIOD_CURRENT)
   {
      ENUM_TIMEFRAMES chartTimeframe = (ENUM_TIMEFRAMES)Period();
      if(chartTimeframe > 0)
         return chartTimeframe;
      return PERIOD_M5;
   }
   return BreakoutTimeframe;
}
...
void DeactivatePCMSetup(string reason = "")
{
   ...
   if(hadSetup && IsPCMVerboseEnabled())
   {
      ...
      Print("INFO: setup PCM desativado. motivo=", resolvedReason,
                                 ...
                                 string scheduleReason = "TP")
{
   ...
   {
      Print("INFO: PCM nao armada (origem=", scheduleReason, ") - runtime desabilitado.");
      return false;
   }
   if(g_tradePCM && !allowCurrentPCMContext)
   {
      Print("INFO: PCM nao armada (origem=", scheduleReason, ") - contexto PCM atual nao permitido.");
      return false;
   }
   if(g_pcmPendingActivation || g_pcmReady)
   {
      Print("INFO: PCM armada (origem=", scheduleReason,
   ...
   {
      Print("INFO: PCM nao armada (origem=", scheduleReason, ") - max operacoes/dia atingido. ops=",
   ...
   Print("INFO: PCM armado (origem=", scheduleReason, "). Inicio do novo CA em ",
...
bool TryActivatePCMChannel()
{
   ...
   {
      Print(" PCM cancelado - Range pequeno: ", g_channelRange, " < ", pcmMinRange);
   ...
   {
      Print(" PCM em modo SLD. Range=", g_channelRange, " > ", pcmSlicedThreshold);
   ...
   {
      Print(" PCM cancelado - Range grande: ", g_channelRange, " > ", pcmMaxRange);
      g_channelCalculated = true;
      g_channelValid = false;
                                    ...
                                    datetime channelDefinitionTime)
{
                                       ...
                                       equivalentPreArmedTicket))
   {
      ...
      Print("INFO: virada pre-armada equivalente ja existente, estado adotado. Ticket=", equivalentPreArmedTicket);
      return true;
   }
   ...
   if(!IsProjectedDrawdownWithinLimits("PlacePreArmedReversalStopOrder",
                                       ...
                                       true))
   {
      Print("INFO: virada pre-armada bloqueada por DD+LIMIT projetado.");
      return false;
   }
...
int OnInit()
{
   ...
   if(restoredDDState)
   {
      Print("INFO: estado de DD restaurado da persistencia. day_balance=", DoubleToString(g_dayStartBalance, 2),
   ...
   else
   {
      ...
      if(g_initialAccountBalance <= 0.0)
         g_initialAccountBalance = (configuredInitialDeposit > 0.0) ? configuredInitialDeposit : currentBalance;
      Print("INFO: estado de DD inicializado sem persistencia previa.");
   }
   ...
   if(g_negativeAddTPAdjustRuntimeEnabled)
   {
      ...
      {
         Print("INFO: Ajuste de TP apos addon ativo somente para contexto PCM.");
      }
   }
   ...
   if(OpeningChannelBars <= 0)
      Print("WARN: OpeningChannelBars <= 0. Sera usado 4 em runtime.");
   ...
   if(EntryVerboseIntervalSeconds <= 0)
      Print("WARN: EntryVerboseIntervalSeconds <= 0. Sera usado intervalo minimo de 1s.");
   ...
   Print("Deposito inicial (parametro): ", DoubleToString(InitialDepositReference, 2), " (0 = automatico)");
   ...
   Print("Forcar DD no saldo dia se abaixo do deposito inicial: ", ForceDayBalanceDDWhenUnderInitialDeposit ? "Sim" : "Nao");
   Print("Referencia DD percentual: ", DrawdownPercentReferenceToString());
   Print("Referencia DD percentual efetiva: ", ResolveEffectiveDrawdownReferenceForLogs());
   Print("Verbose DD logs: ", EnableVerboseDDLogs ? "Ativo" : "Inativo",
   ...
   Print("Timeframe rompimento entrada principal: ", EnumToString(ResolveMainBreakoutTimeframe()),
         " (param=", EnumToString(BreakoutTimeframe), ")");
   Print("Barras do canal de abertura: ", ResolveOpeningChannelBars());
   Print("Verbose entrada logs: ", EnableEntryVerboseLogs ? "Ativo" : "Inativo",
         " | intervalo(s)=", ResolveEntryVerboseIntervalSeconds());
   ...
   Print("Distancia TP apos addon em PCM (% da dist. ate SL): ", PCMNegativeAddTPDistancePercent, "%");
   ...
   Print("PCM - gatilho Break even (% TP): ", PCMBreakEvenTriggerPercent);
   Print("PCM - Trailling stop: ", TraillingStop ? "Ativo" : "Inativo");
   Print("PCM - reducao TP (%): ", PCMTPReductionPercent);
   Print("PCM - usar range principal: ", PCMUseMainChannelRangeParams ? "Sim" : "Nao");
   Print("PCM - range minimo efetivo: ", ResolvePCMMinChannelRange());
   Print("PCM - range maximo efetivo: ", ResolvePCMMaxChannelRange());
   Print("PCM - threshold SLD efetivo: ", ResolvePCMSlicedThreshold());
   ...
   Print("PCM - verbose logs: ", EnablePCMVerboseLogs ? "Ativo" : "Inativo");
   Print("PCM - intervalo verbose (s): ", PCMVerboseIntervalSeconds);
   Print("Operar com Overnight: ", AllowTradeWithOvernight ? "Sim" : "Nao");
   Print("Manter posicoes overnight: ", KeepPositionsOvernight ? "Sim" : "Nao");
   Print("Manter posicoes no fim de semana: ", KeepPositionsOverWeekend ? "Sim" : "Nao");
   ...
   Print("Filling mode ativo: ", EnumToString(g_tradeFillingMode));
...
void ProcessStrategy()
{
   ...
   if(!g_channelValid)
   {
      PrintEntryVerboseRateLimited("entrada bloqueada: canal invalido para operacao.",
                                   g_entryVerboseLastChannelInvalidLogTime);
      return;
   }
   ...
   else
   {
      ...
      {
         PrintEntryVerboseRateLimited("entrada bloqueada por FirstEntryMaxHour na estrategia principal. agora=" +
                                      StringFormat("%02d:%02d", time.hour, time.min) +
                                      " | FirstEntryMaxHour=" + IntegerToString(FirstEntryMaxHour),
                                      g_entryVerboseLastFirstEntryHourBlockLogTime);
         return;
      }
   }
...
void CalculateOpeningChannel()
{
   ...
   int requiredBars = ResolveOpeningChannelBars();
   int barsFound = 0;
   double channelHigh = 0.0;
   double channelLow = 0.0;
   bool channelInitialized = false;
   datetime lastChannelBarTime = 0;
   ...
   for(int i = totalBars - 1; i >= 0; i--)
   {
      ...
      if(barTime >= dayStart)
      {
         double barHigh = iHigh(_Symbol, g_activeTimeframe, i);
         double barLow = iLow(_Symbol, g_activeTimeframe, i);
         ...
         {
            if(barHigh > channelHigh)
               channelHigh = barHigh;
            if(barLow < channelLow)
               channelLow = barLow;
         }
         ...
         {
            lastChannelBarTime = barTime;
            break;
         }
      }
   }

   // Verificar se as velas do canal estao fechadas
   if(barsFound < requiredBars)
      return;

   datetime channelCloseTime = lastChannelBarTime + PeriodSeconds(g_activeTimeframe);
   if(currentTime < channelCloseTime)
   {
      Print(" Aguardando fechamento da vela #", requiredBars,
            " do canal... Fecha as: ", TimeToString(channelCloseTime, TIME_MINUTES));
      return;
   }

   // Timestamp efetivo de definicao do canal (fechamento da ultima vela considerada)
   g_channelDefinitionTime = channelCloseTime;

   // Calcular range
   g_channelHigh = channelHigh;
   g_channelLow = channelLow;
   g_channelRange = g_channelHigh - g_channelLow;

   Print(" Canal calculado com ", requiredBars, " velas:");
   Print("  High: ", g_channelHigh, " | Low: ", g_channelLow, " | Range: ", g_channelRange);
...
void DetectBreakout()
{
   ...
   ENUM_TIMEFRAMES breakoutTimeframe = ResolveMainBreakoutTimeframe();
   if(isPCMEntry)
   ...
   if(tolDigits < 5)
      tolDigits = 5;
   string tfLabel = EnumToString(breakoutTimeframe);
   ...
   if(g_cycle1Direction == "BOTH")
   {
      ...
      string slicedNoBreakoutMsg = "sem rompimento (SLICED). close=" + DoubleToString(close, digits) +
                                   " | gatilho_compra>" + DoubleToString(g_cycle1High + breakoutTolerance, tolDigits) +
                                   " | gatilho_venda<" + DoubleToString(g_cycle1Low - breakoutTolerance, tolDigits) +
                                   " | tf=" + tfLabel;
      PrintEntryVerboseRateLimited(slicedNoBreakoutMsg, g_entryVerboseLastNoBreakoutLogTime);
      return;
   }
   ...
   if(!g_cycle1Defined)
   {
      ...
      string c1NoBreakoutMsg = "sem rompimento para definir C1. close=" + DoubleToString(close, digits) +
                               " | gatilho_up>" + DoubleToString(g_channelHigh + breakoutTolerance, tolDigits) +
                               " | gatilho_down<" + DoubleToString(g_channelLow - breakoutTolerance, tolDigits) +
                               " | tf=" + tfLabel;
      PrintEntryVerboseRateLimited(c1NoBreakoutMsg, g_entryVerboseLastNoBreakoutLogTime);
      return;
   }
   ...
   if(g_cycle1Direction == "UP")
   {
      ...
      string upNoBreakoutMsg = "sem rompimento em C1=UP. close=" + DoubleToString(close, digits) +
                               " | cont_up>" + DoubleToString(g_cycle1High + breakoutTolerance, tolDigits) +
                               " | rev_down<" + DoubleToString(g_channelLow - breakoutTolerance, tolDigits) +
                               " | tf=" + tfLabel;
      PrintEntryVerboseRateLimited(upNoBreakoutMsg, g_entryVerboseLastNoBreakoutLogTime);
   }
   else if(g_cycle1Direction == "DOWN")
   {
      ...
      string downNoBreakoutMsg = "sem rompimento em C1=DOWN. close=" + DoubleToString(close, digits) +
                                 " | cont_down<" + DoubleToString(g_cycle1Low - breakoutTolerance, tolDigits) +
                                 " | rev_up>" + DoubleToString(g_channelHigh + breakoutTolerance, tolDigits) +
                                 " | tf=" + tfLabel;
      PrintEntryVerboseRateLimited(downNoBreakoutMsg, g_entryVerboseLastNoBreakoutLogTime);
   }
}
...
void ExecuteMarketOrder(ENUM_ORDER_TYPE orderType, double price, double stopLoss, double takeProfit, bool isPCMContext = false)
{
   ...
   if(!IsProjectedDrawdownWithinLimits("ExecuteMarketOrder",
                                       ...
                                       true))
   {
      Print(" Ordem a mercado bloqueada por DD+LIMIT projetado.");
      return;
   }
                     ...
                     bool isPCMContext = false)
{
   ...
   if(g_pendingLimitRetryArmed)
   {
      Print("INFO: PlaceLimitOrder ignorado: fila de reenvio LIMIT ja armada.");
      return;
   }
                                       ...
                                       equivalentPendingTicket))
   {
      RegisterMainPendingOrderState(equivalentPendingTicket,
                                    ...
                                    isPCMContext,
                                    limitPrice);
      Print("INFO: ordem limite equivalente ja existente, estado adotado. Ticket: ", equivalentPendingTicket);
      return;
   }
   ...
   if(!IsProjectedDrawdownWithinLimits("PlaceLimitOrder",
                                       ...
                                       true))
   {
      Print(" Ordem limite bloqueada por DD+LIMIT projetado. prioridade=", ddCandidatePriority);
      return;
   }
...
void CheckPendingOrderExecution()
{
   ...
   if(isReversalPending)
   {
      ...
      if(!IsReversalAllowedByEntryHourNow())
      {
         ...
         if(g_pendingOrderIsPCM)
            Print("INFO: cancelamento de ordem PCM por horario limite de entrada.");
   ...
   if(IsDrawdownLimitReached("CheckPendingOrderExecution") && OrderSelect(g_currentTicket))
   {
      ...
      if(g_pendingOrderIsPCM)
         Print("INFO: cancelamento de ordem PCM por limite de DD.");
   ...
   if(targetReachedBeforeFill)
   {
      ...
      ConsumeCycleAfterPendingCancel();
      if(g_pendingOrderIsPCM)
         Print("INFO: cancelamento de ordem PCM porque alvo projetado foi atingido antes da execucao.");
   ...
   ConsumeCycleAfterPendingCancel();
   if(g_pendingOrderIsPCM)
      Print("INFO: cancelamento de ordem PCM porque ticket pendente nao foi mais encontrado.");
...
void LogDailyDDOpeningStatus(string context)
{
   ...
   Print("DD OPEN [", context, "] day_balance=", DoubleToString(dayBalance, 2),
   ...
   Print("DD OPEN SHORT [", context, "] day_balance=", DoubleToString(dayBalance, 2),
         " | max_dd_daily_limit=", DoubleToString(dailyLimit, 2),
         " | max_dd_limit=", DoubleToString(maxLimit, 2));
}
...
bool IsOrderIntentBlocked(string key, string context, int cooldownSeconds = 5)
{
   ...
   for(int i = 0; i < total; i++)
   {
      ...
      {
         Print("SEND_GUARD bloqueou repeticao [", context, "] key=", key,
...
int ResolvePendingOrderQueuePriority(ulong ticket)
{
   ...
   if(symbol == _Symbol && magic == MagicNumber)
   {
      if(StringFind(comment, "AddOn Negativo") >= 0)
         return DD_QUEUE_ADON;
      if(StringFind(comment, "Virada") >= 0 || StringFind(comment, "turnof") >= 0)
         return DD_QUEUE_TURNOF;
      return DD_QUEUE_FIRST;
   }
...
void RecoverRuntimeStateFromBroker(string context = "runtime")
{
   ...
   for(int i = 0; i < totalPositions; i++)
   {
      ...
      if(latestPositionTicket == 0 || posTime >= latestPositionTime)
      {
         ...
         latestPositionIsPCM = ContainsTextInsensitive(posComment, "pcm");
      }
   }
   ...
   for(int i = totalOrders - 1; i >= 0; i--)
   {
      ...
      if(ContainsTextInsensitive(orderComment, "prearmada"))
      {
      ...
      bool isPCM = ContainsTextInsensitive(orderComment, "pcm");
   ...
   {
      Print("INFO: recuperacao de estado concluida [", context, "]",
                            ...
                            bool forceLog = false)
{
   ...
   Print("DD VERBOSE [", context, "] ref=", ResolveEffectiveDrawdownReferenceForLogs(),
   ...
   Print("DD VERBOSE [", context, "] open_additional=", DoubleToString(openAdditionalRisk, 2),
   ...
   Print("DD VERBOSE [", context, "] candidate=", candidatePriorityText,
...
bool CancelPendingOrderByTicketForDDQueue(ulong ticket, string reason)
{
   ...
   Print("INFO: DD queue cancelou ordem. Ticket=", ticket, " | motivo=", reason);
                                     ...
                                     bool forceVerboseLogOnBlock)
{
   ...
   while(true)
   {
      ...
      if(openMissingSLCount > 0 || pendingMissingSLCount > 0)
      {
         ...
         Print("BLOQUEIO DD+LIMIT [", context, "]",
...
bool EnforceProjectedDrawdownAfterPositionOpen(string context, ulong openedTicket)
{
   ...
   {
      Print("INFO: Posicao fechada por DD+LIMIT projetado apos abertura. contexto=", context,
            " | ticket=", openedTicket);
   }
   else
   {
...
bool ClosePositionByTicketMarket(ulong ticket, string reason)
{
   ...
   {
      if(reason == "")
         Print("INFO: Posicao fechada a mercado. Ticket=", ticket);
      else
         Print("INFO: Posicao fechada a mercado (", reason, "). Ticket=", ticket);
      return true;
   }
   ...
   // A partir daqui a posicao ja foi fechada.
   {
      ...
      if(g_firstTradeExecuted && !g_reversalTradeExecuted)
      {
         if(wasStopLoss)
         {
            if(g_tradePCM)
            {
               ...
               if(pcmScheduledFromSL)
                  Print(" STOP LOSS ATINGIDO - Operacao PCM sem turnof. PCM armado para nova operacao.");
               else
         ...
         else
         {
            ...
            {
               if(g_tradePCM)
                  Print(" Take Profit atingido - Operacao PCM finalizada. PCM armado para nova operacao.");
               else
...
bool CheckIfStopLossHitRobust()
{
   ...
   Print(" Metodo 3: Fallback - assumindo SL");
   return true;
}
...
void ExecuteReversal()
{
   ...
   if(!IsProjectedDrawdownWithinLimits("ExecuteReversal",
                                       ...
                                       true))
   {
      Print(" turnof mercado bloqueada por DD+LIMIT projetado.");
      return;
   }
   ...
   Print("  Result: ", result, " | ResultRetcode: ", trade.ResultRetcode());
                                 ...
                                 int sourceOperationChainId = 0)
{
   ...
   if(!IsProjectedDrawdownWithinLimits("TryExecuteOvernightReversal",
                                       ...
                                       true))
   {
      Print(" Virada overnight a mercado bloqueada por DD+LIMIT projetado.");
      return false;
   }
...
string BuildRunConfigJson(datetime startTime, datetime endTime)
{
   ...
   cfg += "      \"OpeningChannelBars\": " + IntegerToString(ResolveOpeningChannelBars()) + ",\n";
   ...
   cfg += "      \"BreakoutTimeframe\": \"" + EnumToString(ResolveMainBreakoutTimeframe()) + "\",\n";
   cfg += "      \"BreakoutMinTolerancePoints\": " + DoubleToString(BreakoutMinTolerancePoints, 2) + ",\n";
   cfg += "      \"EnableEntryVerboseLogs\": " + BoolToJson(EnableEntryVerboseLogs) + ",\n";
   cfg += "      \"EntryVerboseIntervalSeconds\": " + IntegerToString(ResolveEntryVerboseIntervalSeconds()) + ",\n";
