...
input double   BreakoutMinTolerancePoints = 10.0; // Tolerancia minima (em pontos) para confirmar rompimento
...
input double   RiskPercent = 3.0;           // Risco por operacao (%)
...
input double   MaxDailyDrawdownPercent = 0.0; // Limite de DD diario em % (0 desativa)
input double   MaxDrawdownPercent = 0.0;      // Limite de DD maximo em % (0 desativa)
...
input bool     EnablePCMOnNoTradeLimitTarget = true; // Habilitar PCM em NoTrade quando LIMIT cancela por alvo projetado
input bool     BreakEven = true;            // Break even
input bool     TraillingStop = true;        // Trailling stop
input double   PCMTPReductionPercent = 10.0; // Reducao do TP (%) somente para PCM
input double   PCMRiskPercent = 0.5;         // Risco por operacao (%) para entrada PCM
...
input bool     EnablePCMHourLimit = true;   // Habilitar horario limite especifico para entrada PCM
input int      PCMEntryMaxHour = 16;         // Hora limite para entrada PCM
input int      PCMEntryMaxMinute = 0;       // Minuto limite para entrada PCM
...
string g_programName = "hantec_cents";
                            ...
                            bool forceLog = false)
{
   ...
   Print("DD VERBOSE [", context, "] ref=", DrawdownPercentReferenceToString(),
                                     ...
                                     bool forceVerboseLogOnBlock)
{
   ...
   while(true)
   {
      ...
      LogVerboseDDProjection(context,
                             ...
                             pendingMissingSLCount,
                             pendingOrdersCount,
                             forceVerboseLogOnBlock && (dailyExceeded || maxExceeded));

      if(!dailyExceeded && !maxExceeded)
         return true;
      ...
      Print("BLOQUEIO DD+LIMIT [", context, "]",
            ...
            " | projected_max=", DoubleToString(projectedMaxAmount, 2),
            " | max_limit=", DoubleToString(maxLimit, 2),
            " | ref=", DrawdownPercentReferenceToString());
      return false;
   }
