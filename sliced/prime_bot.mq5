...
void ExecuteMarketOrder(ENUM_ORDER_TYPE orderType, double price, double stopLoss, double takeProfit)
{
   ...
   if(orderType == ORDER_TYPE_BUY)
      result = trade.Buy(lotSize, _Symbol, 0, stopLoss, takeProfit, "Primeira Operacao");
   else
      result = trade.Sell(lotSize, _Symbol, 0, stopLoss, takeProfit, "Primeira Operacao");
...
bool ApplyNoOvernightPolicy()
{
   ...
   if(g_pendingOrderPlaced && g_currentTicket > 0 && OrderSelect(g_currentTicket))
   {
      if(trade.OrderDelete(g_currentTicket))
      {
         Print("INFO: Ordem pendente cancelada por politica sem overnight. Ticket=", g_currentTicket);
         ConsumeCycleAfterPendingCancel();
         ClearPendingOrderState(true);
         g_tradeEntryTime = 0;
         g_tradeReversal = false;
