...
double   FixedLotAllEntries = 0.0;
double   RiskPercent = 0.7;
bool     UseInitialDepositForRisk = true;
...
int      MaxEntryHour = 16;           // Horario limite para entrada
...
double   BreakoutTolerancePoints = 10.0; // Tolerancia minima para confirmar rompimento (pontos)
...
double   NegativeAddLotMultiplier = 0.60;  // Multiplicador do lote base na adicao
...
bool     EnableLicenseValidation = true;       // Validar licenca por conta (obrigatorio no release)
string   LicenseServerBaseUrl = "";           // URL base do servidor de licenca (https://...)
string   LicenseToken = "";                   // Token de licenca do cliente
string   LicensedCustomerName = "Italo";      // Nome do cliente para watermark
int      LicenseCheckIntervalMinutes = 10;    // Intervalo de revalidacao
int      LicenseGracePeriodHours = 24;        // Janela offline apos ultima validacao
bool     EnforceLiveHedgeAccount = true;      // Permitir apenas conta real em modo hedge
...
int g_drawdownLastBlockType = 0;  // 1=daily%, 2=max%, 3=daily$, 4=max$, 5=missing_sl
...
bool g_sanitizeClientJsonLogs = true;
...
string NormalizeDirectionLabel(ENUM_ORDER_TYPE orderType);
...
string BuildLicenseValidationUrl()
{
   ...
   // Se cliente passar endpoint completo, usa como esta.
...
bool TryParseLicenseExpiryTimestamp(string rawValue, datetime &parsedTs)
{
   ...
   int minusPos = StringFind(value, "-", 12); // ignora separador de data em YYYY-MM-DD
   if(minusPos > 0)
...
bool ValidateLicenseOnline(bool logErrors = false)
{
   ...
   if(httpCode < 0)
   {
      ...
      if(logErrors)
         ReleaseError("LIC_HTTP", "WebRequest falhou. erro=" + IntegerToString(err));
      return false;
   }
...
bool ValidateLicenseAndApplyPolicy(bool logErrors = false)
{
   if(!EnableLicenseValidation)
   {
      ...
      g_licenseMessage = "license_validation_mandatory";
      if(logErrors)
         ReleaseError("LIC_RELEASE", "Validacao de licenca obrigatoria no release.");
      return false;
   }
   ...
   if(g_licenseExpired)
   {
      ...
      if(logErrors)
      {
         string expTxt = (g_licenseExpiresAt > 0) ? TimeToString(g_licenseExpiresAt, TIME_DATE|TIME_SECONDS) : "indefinido";
         ReleaseError("LIC_EXPIRED", "Licenca expirada. expires_at=" + expTxt);
      }
      return false;
   }
   ...
   g_securityBlockTrading = true;
   if(logErrors)
      ReleaseError("LIC_BLOCK", "Licenca invalida/indisponivel e fora da janela de grace.");
   return false;
}
...
bool EnforceAccountSecurity()
{
   ...
   {
      ReleaseError("ACCOUNT_MODE", "InsideBot permitido apenas em conta real.");
      return false;
   }

   if(!g_isHedgingAccount)
   {
      ReleaseError("MARGIN_MODE", "InsideBot permitido apenas em conta hedge.");
      return false;
   }
...
string BuildOperationCode(bool isReversal, int chainId)
{
                                    ...
                                    datetime channelDefinitionTime)
{
   ...
   if(baseOrderType != ORDER_TYPE_BUY && baseOrderType != ORDER_TYPE_SELL)
      return false;
   if(baseStopLoss <= 0.0 || channelRange <= 0.0 || (!IsFixedLotAllEntriesEnabled() && lotSize <= 0.0))
      return false;
   if(g_tradeReversal)
      return false;
   ...
   if(reversalType == ORDER_TYPE_BUY)
      result = trade.BuyStop(lotSize, entryPrice, _Symbol, revStopLoss, revTakeProfit, ORDER_TIME_GTC, 0, "");
   else
      result = trade.SellStop(lotSize, entryPrice, _Symbol, revStopLoss, revTakeProfit, ORDER_TIME_GTC, 0, "");

   long retcode = trade.ResultRetcode();
   if(result && trade.ResultOrder() > 0)
   {
...
int OnInit()
{
   ...
   g_releaseInfoLogsEnabled = false;
   g_sanitizeClientJsonLogs = true;
   ...
   // Evita reset imediato apenas quando o estado persistido pertence ao mesmo dia.
   ...
   else
   {
      if(!EnableLicenseValidation)
      {
         ReleaseError("LIC_RELEASE", "EnableLicenseValidation deve permanecer true em release.");
         return(INIT_FAILED);
      }
   ...
   if(g_negativeAddTPAdjustRuntimeEnabled)
   {
      ...
      {
         g_negativeAddTPAdjustRuntimeEnabled = false;
         if(g_releaseInfoLogsEnabled) Print("WARN: Ajuste de TP apos addon desabilitado. NegativeAddTPDistancePercent <= 0.");
      }
   }
...
void OnTick()
{
   ...
   if(!g_isTesterMode)
   {
      ...
      if(g_securityBlockTrading)
      {
         ...
         {
            ReleaseError("LIC_RUNTIME_BLOCK", "Execucao bloqueada. status=" + g_licenseStatus + " msg=" + g_licenseMessage);
...
void ProcessStrategy()
{
   ...
   if(g_firstTradeExecuted) return;
...
void DrawChannelLines()
{
   ...
   ChartRedraw();
   if(g_releaseInfoLogsEnabled) Print(" Canais desenhados");
}
...
void DetectBreakout()
{
   ...
   double tolPrice = 0.0;
   if(BreakoutTolerancePoints > 0.0 && g_pointValue > 0.0)
      tolPrice = BreakoutTolerancePoints * g_pointValue;

   // Modo SLICED: C1 ja definido como BOTH
   if(g_cycle1Direction == "BOTH")
   {
      // Rompimento para cima
      if(close > (g_cycle1High + tolPrice))
      {
   ...
   {
      // Rompimento para cima
      if(close > (g_channelHigh + tolPrice))
      {
      ...
      // Rompimento para baixo
      if(close < (g_channelLow - tolPrice))
      {
...
void OpenFirstPosition(ENUM_ORDER_TYPE orderType)
{
   ...
   {
      ExecuteMarketOrder(orderType, price, stopLoss, takeProfit);
   }
   else
   {
      PlaceLimitOrder(orderType,
...
//+------------------------------------------------------------------+
void ExecuteMarketOrder(ENUM_ORDER_TYPE orderType, double price, double stopLoss, double takeProfit)
{
   ...
   bool result = false;
   if(orderType == ORDER_TYPE_BUY)
      result = trade.Buy(lotSize, _Symbol, 0, stopLoss, takeProfit, "");
   else
      result = trade.Sell(lotSize, _Symbol, 0, stopLoss, takeProfit, "");
                     ...
                     double channelRangeContext = 0.0)
{
   ...
   if(IsFixedLotAllEntriesEnabled())
      ...
      lotSize = CalculateLotSize(limitPrice, stopLoss);
...
double CalculateLotSize(double entryPrice, double stopLoss)
{
   ...
   double riskAmount = riskReferenceBalance * (RiskPercent / 100.0);
...
string BuildDrawdownStateKey(string suffix)
{
   ...
   string key = "IBDD_" + IntegerToString((int)login) + "_" + _Symbol + "_" +
                magicText + "_" + suffix;

   // Global variable names possuem limite curto; aplica fallback sem simbolo se necessario.
   if(StringLen(key) >= 63)
      key = "IBDD_" + IntegerToString((int)login) + "_" + magicText + "_" + suffix;
...
bool RestoreDrawdownStateFromPersistence()
{
   ...
   if(storedDayKey == currentDayKey && storedDayEquity > 0.0)
   {
...
bool HasAnyEAExposureWithoutSL(int &openMissingSLCount, int &pendingMissingSLCount)
{
   ...
   return (openMissingSLCount > 0 || pendingMissingSLCount > 0);
}
...
bool IsDrawdownLimitReached(string context = "")
{
   ...
   if(HasAnyEAExposureWithoutSL(openMissingSLCount, pendingMissingSLCount))
   {
      datetime now = TimeCurrent();
      bool shouldLog = (g_drawdownLastBlockType != 5) ||
                       (g_drawdownLastBlockContext != context) ||
                       ((now - g_drawdownLastBlockLogTime) >= 60);
      if(shouldLog)
      {
         Print("BLOQUEIO DD+LIMIT", (context == "" ? "" : " [" + context + "]"),
...
bool IsNegativeAddTicket(ulong ticket)
{
   ...
   int dealsTotal = HistoryDealsTotal();
   bool runtimeAddOnTicket = (FindNegativeAddExecutedTicket(ticket) >= 0);
   for(int i = 0; i < dealsTotal; i++)
   {
      ulong dealTicket = HistoryDealGetTicket(i);
      if(dealTicket == 0)
   ...
   for(int t = 0; t < ticketCount; t++)
   {
      ...
      for(int i = 0; i < dealsTotal; i++)
      {
         ...
         if(dealEntry == DEAL_ENTRY_IN)
         {
            ...
            bool isAddOnEntry = runtimeAddOnTicket || IsNegativeAddComment(dealComment) || IsNegativeAddComment(orderComment);
            if(isAddOnEntry)
            {
...
bool ClosePositionByTicketMarket(ulong ticket)
{
   ...
   if(result && (retcode == TRADE_RETCODE_DONE || retcode == TRADE_RETCODE_DONE_PARTIAL))
   {
      if(g_releaseInfoLogsEnabled) Print("INFO: Posicao fechada por politica sem overnight. Ticket=", ticket);
      return true;
   }
...
bool ApplyNoOvernightPolicy()
{
   ...
   datetime nowTime = TimeCurrent();
   bool inCloseWindow = hasSession && nowTime >= triggerTime && nowTime < marketClose;
   ...
   if(!inCloseWindow && !hasForcedOvernight)
      return false;

   if(g_pendingOrderPlaced && g_currentTicket > 0 && OrderSelect(g_currentTicket))
   {
      if(trade.OrderDelete(g_currentTicket))
      {
         if(g_releaseInfoLogsEnabled) Print("INFO: Ordem pendente cancelada por politica sem overnight. Ticket=", g_currentTicket);
         ConsumeCycleAfterPendingCancel();
         ClearPendingOrderState(true);
         g_tradeEntryTime = 0;
         g_tradeReversal = false;
   ...
   {
      ArrayResize(ticketsToClose, ArraySize(ticketsToClose) + 1);
      ticketsToClose[ArraySize(ticketsToClose) - 1] = g_currentTicket;
   }

   if(g_overnightTicket > 0)
   {
      bool exists = false;
      for(int i = 0; i < ArraySize(ticketsToClose); i++)
      {
         if(ticketsToClose[i] == g_overnightTicket)
         {
            exists = true;
            break;
         }
      ...
      {
         ArrayResize(ticketsToClose, ArraySize(ticketsToClose) + 1);
         ticketsToClose[ArraySize(ticketsToClose) - 1] = g_overnightTicket;
      }
   }

   for(int i = 0; i < ArraySize(g_overnightLogTickets); i++)
   {
      ulong t = g_overnightLogTickets[i];
      if(t == 0)
         continue;

      bool exists = false;
      for(int j = 0; j < ArraySize(ticketsToClose); j++)
      {
         if(ticketsToClose[j] == t)
         {
            exists = true;
            break;
         }
      ...
      {
         ArrayResize(ticketsToClose, ArraySize(ticketsToClose) + 1);
         ticketsToClose[ArraySize(ticketsToClose) - 1] = t;
      }
   }
   ...
   for(int i = 0; i < ArraySize(ticketsToClose); i++)
   {
      if(ClosePositionByTicketMarket(ticketsToClose[i]))
         attemptedClose = true;
   }
...
void CheckPositionStatus()
{
   ...
   for(int i = ArraySize(g_overnightLogTickets) - 1; i >= 0; i--)
   {
      ...
      if(!PositionSelectByTicket(overnightTicket))
      {
         ...
         if(overnightLoggedFromList)
         {
            ...
            if(overnightWasMainTicket && overnightWasStopLoss)
            {
   ...
   if(activeCount > 0)
   {
      ...
      if(!g_tradeReversal && !AllowReversalAfterMaxEntryHour && IsAfterMaxEntryHour())
      {
         if(g_preArmedReversalOrderTicket > 0)
            CancelPreArmedReversalOrder("horario limite de entrada atingido");
         if(!g_reversalBlockedByEntryHour)
      ...
      if(!g_tradeReversal && g_preArmedReversalOrderTicket == 0)
         EnsurePreArmedReversalForCurrentTrade();
...
void ResetDaily()
{
   ...
   if(g_currentTicket > 0 && PositionSelectByTicket(g_currentTicket))
   {
      ...
      if(EnableOvernightReversal && !g_tradeReversal && g_preArmedReversalOrderTicket == 0)
      {
         EnsurePreArmedReversalForCurrentTrade();
      }
...
string BuildRunConfigJson(datetime startTime, datetime endTime)
{
   ...
   cfg += "    \"security\": {\n";
   cfg += "      \"enforce_live_hedge\": " + BoolToJson(EnforceLiveHedgeAccount) + ",\n";
   cfg += "      \"license_validation\": " + BoolToJson(EnableLicenseValidation) + ",\n";
   cfg += "      \"license_status\": \"" + g_licenseStatus + "\",\n";
   cfg += "      \"license_revoked\": " + BoolToJson(g_licenseRevoked) + "\n";
   cfg += "    },\n";

   if(g_sanitizeClientJsonLogs)
   {
      cfg += "    \"selected_parameters\": {\n";
      cfg += "      \"EnableLogging\": " + BoolToJson(EnableLogging) + ",\n";
      cfg += "      \"SanitizeClientJsonLogs\": " + BoolToJson(g_sanitizeClientJsonLogs) + "\n";
      cfg += "    }\n";
      cfg += "  }";
      return cfg;
   }
...
void SaveLogs()
{
   ...
   if(StringLen(g_tradesLog) > 15)
   {
      ...
      if(handle != INVALID_HANDLE)
      {
         ...
         ReleaseError("LOG_WRITE_TRADES", "falha ao salvar arquivo de trades. erro=" + IntegerToString(GetLastError()));
   }
   ...
   if(StringLen(g_noTradesLog) > 22)
   {
      ...
      if(handle != INVALID_HANDLE)
      {
         ...
         ReleaseError("LOG_WRITE_NOTRADES", "falha ao salvar arquivo de no-trades. erro=" + IntegerToString(GetLastError()));
   }
}
              ...
              bool hasFinancialBreakdownOverride = false)
{
   ...
   bool isFirstOperation = (!isTurnOperation && !isAddOperation);
   string operationChainCode = BuildOperationChainCode(operationChainId);
   string operationCode = BuildOperationCode(isTurnOperation, operationChainId);
   ...
   if(g_sanitizeClientJsonLogs)
   {
      ...
      tradeJson += "  \"direction\": \"" + NormalizeDirectionLabel(g_currentOrderType) + "\",\n";
   ...
   else
   {
      ...
      tradeJson += "  \"is_add_operation\": " + (isAddOperation ? "true" : "false") + ",\n";
      tradeJson += "  \"add_operation_code\": \"" + addOperationCode + "\",\n";
      tradeJson += "  \"timeframe\": \"" + EnumToString(g_activeTimeframe) + "\",\n";
